...
//            return $this->db->get('mst_media')->result();
        ...
        function checkLogin($user, $pass){
            $count = $this->db->select("user.user_ID, user.username")
//                        ->join('mst_user_type', 'mst_user_type.user_type_ID = mst_user.user_type_ID', 'inner')
...
//            return $this->db->get('mst_media')->result();
        ...
        function getPermission($userId, $class){
       
                            
                            
            $permission = $this->db
            ->query("SELECT * , case when t.menu_page_link = '$class/' then '1' else '0' end as is_selected ".
                "FROM ( ".
                        
                    "SELECT DISTINCT mp.menu_page_code, mp.menu_page_name, mp.menu_page_link, mp.icon, tug.TRN_USER_GROUP_SEQ, up.USER_PERMISSION_SEQ ".
                    "FROM user u ".
                    "JOIN trn_user_group tug on (u.user_id = tug.user_id) ".
                    "JOIN user_permission up on (tug.user_group_id = up.user_group_id) ".
                    "JOIN menu_page mp on (up.MENU_PAGE_ID = mp.MENU_PAGE_ID) ".
                    "JOIN ( ".
                            "SELECT u.user_id, up.MENU_PAGE_ID, MIN(tug.TRN_USER_GROUP_SEQ) as TRN_USER_GROUP_SEQ ".
                                    "FROM user u ".
                                    "JOIN trn_user_group tug ON (u.user_id = tug.user_id) ".
                                    "JOIN user_permission up ON (tug.user_group_id = up.user_group_id) ".
                                    "WHERE u.user_id = $userId ".
                            "GROUP BY up.MENU_PAGE_ID ".
                    ") g1 on (g1.MENU_PAGE_ID = mp.MENU_PAGE_ID AND g1.TRN_USER_GROUP_SEQ = tug.TRN_USER_GROUP_SEQ) ".
                    "WHERE u.user_id = $userId ".
                    
                    "UNION  ".

                    "SELECT DISTINCT  ".
                    "mpp.menu_page_code, mpp.menu_page_name, mpp.menu_page_link, mpp.icon, '-1' as TRN_USER_GROUP_SEQ, '-1' as USER_PERMISSION_SEQ ".
                    "FROM user u ".
                    "JOIN trn_user_group tug on (u.user_id = tug.user_id) ".
                    "JOIN user_permission up on (tug.user_group_id = up.user_group_id) ".
                    "JOIN menu_page mp on (up.MENU_PAGE_ID = mp.MENU_PAGE_ID) ".
                    "JOIN menu_page mpp on (mp.PARENT_ID = mpp.MENU_PAGE_ID OR mpp.MENU_PAGE_CODE = '0000') ".
                    "WHERE u.user_id = $userId ".
                ") t ".

                "order by SUBSTRING(t.menu_page_code, 1, 2), t.TRN_USER_GROUP_SEQ, t.USER_PERMISSION_SEQ ")->result_array();
            return $permission;
//            return $this->db->get('mst_media')->result();
        }
        
        function getParentId($menuArr){
            $permission = $this->db
                ->distinct()
                ->select("PARENT_ID")
                ->where_in("MENU_PAGE_ID" , $menuArr)
                ->where("PARENT_ID IS NOT NULL")
                ->get("MENU_PAGE")
                ->result_array();
            $ret = array();
            foreach ($permission as $value) {
                array_push($ret, $value["PARENT_ID"]);
            }
            return $ret;
//            return $this->db->get('mst_media')->result();
        }
...
//        }
        ...
        public function getNewBorrowCode(){
            $result = $this->db->select_max("BORROW_CODE")
                ->get('borrow')->result_array();
            return ($result[0]['BORROW_CODE'] == null ? "1" : $result[0]['BORROW_CODE']);
        }
        
        public function getStatusIdByCode($statusCode){
            $result = $this->db->select("status_id")
                ->where(array("status_code" => $statusCode))
                ->get('status')
                ->result_array();
        ...
        public function getCountMaterialById($materialId){
            $result = $this->db->select("material_balance")
                ->where(array("material_Id" => $materialId))
                ->get('material')
                ->result_array();
            
            return $result[0]['material_balance'];
        }
}
